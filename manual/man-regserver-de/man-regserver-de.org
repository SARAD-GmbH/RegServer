...
** Lokale Geräte am {{{rs}}}
Der {{{rs}}} muss auch den Trivialfall abdecken,
dass {{{instr_m}}} lokal am {{{ac}}},
auf dem der {{{rs}}} läuft,
angeschlossen sind.

** Minimaler {{{is}}} -- {{{am}}}
Wenn {{{ic}}} mit geringer Rechenkapazität
und nur einem angeschlossenen {{{instr}}}
verwendet werden,
dann reduzieren sich die Funktionen aus Abschnitt [[id:1d3fdb23-f77c-4533-8617-de6dab3291bb][{{{is}}}]] auf:
1. Umsetzung der seriellen Schnittstelle des {{{instr}}} in eine TCP/IP-Schnittstelle,
2. Melden des {{{instr}}} an den {{{rs2}}},
3. Empfang von Reservierungsinformationen vom {{{rs2}}} und Protokollierung,
4. Beantwortung von Anfragen des {{{rs2}}} zum Reservierungszustand,

Die letzten drei Funktionen können räumlich von der ersten getrennt werden.
Der auf dem {{{ic}}} laufende {{{is}}} reduziert sich damit
auf die Umsetzung der seriellen Schnittstelle in eine TCP/IP-Schnittstelle,
eine Funktion,
die viele preiswerte Geräte bereits von Hause aus mitbringen.
Die verbleibenden Funktionen 2, 3 und 4
müssen dann von einer anderen Instanz im LAN erfüllt werden,
die wir {{{am}}} nennen.

Damit können auch {{{ic}}} ohne Python-Unterstützung
mit nur je einem angeschlossenen {{{instr}}}
Mitglieder eines (unsicheren) LAN/VPN-Netzwerkes sein.

Es ist ein {{{am}}} für jedes so angeschlossene {{{instr}}} nötig.
Der {{{am}}} erfordert die manuelle Eingabe der Daten des {{{instr}}}
bei der Erstkonfiguration.
Der {{{am}}} prüft,
ebenso wie der {{{is2}}},
in regelmäßigen Abständen,
ob das {{{instr}}} noch über seinen Socket erreichbar ist.

* Migrationsplan label:migrationsplan
Die in Abschnitt ref:simple und ref:is1rs1 beschriebenen Lösungen
existieren, und funktionieren mit den beschriebenen Einschränkungen.
Die in Abschnitt ref:simple beschriebene einfachste Lösung
arbeitet mit {{{rv}}}, {{{dv}}} und {{{rooms}}}.
Die Lösung aus Abschnitt ref:is1rs1
arbeitet mit {{{dv}}} und {{{rooms}}}, nicht aber mit {{{rv}}}.

Wir haben damit auf jeden Fall eine Lösung,
die wir unseren Kunden anbieten können,
und die wir zusammenfassend als {{{sn1}}} bezeichnen wollen.

Mit {{{sn1}}} wäre noch ein evolutionäre Verbesserung denkbar,
indem für alle {{{app_m}}} das bereits mit {{{rooms}}} demonstrierte
Konzept angewendet wird,
bei dem ausschließlich über Server Sockets der {{{is}}}
auf die {{{instr_m}}} zugegriffen wird.
In {{{dv}}} und {{{rv}}} müsste der Scan der lokalen COM-Ports
dafür deaktiviert werden.
Außerdem wären für einen zuverlässigen Betrieb
{{{rs1}}} und {{{is1}}} so zu modifizieren,
dass sie als Windows-Services im Hintergrund laufen
und im Fehlerfall
automatisch neu gestartet werden.

{{{sn2}}} und {{{snmqtt}}} sind zukünftige Lösungen,
die {{{sn1}}} ablösen sollen.
Beide Lösungen haben, abhängig vom Einsatzszenario,
ihre Existenzberechtigung
und sollen deshalb parallel existieren.
Beide Lösungen haben die gleiche einheitliche Schnittstelle zur {{{app}}}
und erfordern entsprechende Änderungen an allen {{{app_m}}}.

{{{sn2}}} ist die Lösung für kleine Installationen
mit überschaubarem und vertrauenswürdigem Nutzerkreis.

{{{snmqtt}}} ist die Lösung der Wahl
für große, weitverzweigte Netze
mit vielen Nutzern und vielen {{{instr_m}}}
bzw. für Einsatzfälle mit hohen Sicherheitsanforderungen.

Zunächst soll {{{sn2}}}
mit allen erforderlichen Änderungen an den {{{app_m}}} umgesetzt werden,
danach folgt {{{snmqtt}}}.

* {{{sn1}}} -- Evolution                                            :noexport:
Wie im Kapitel ref:migrationsplan festgestellt,
existiert mit dem {{{sn1}}} eine,
wenn auch nicht gute, so doch funktionierende
Lösung für alle aktuellen SARAD-Produkte.

Diese Lösung könnte entsprechend dem folgenden Plan
noch erheblich verbessert werden.
Es ist zu diskutieren,
ob sich dieser Aufwand lohnt
oder ob wir davon zu Gunsten der schnelleren Entwicklung des {{{sn2}}}
absehen wollen.

** Grundidee
Die Grundidee ist,
so wie bereits mit {{{rooms}}} demonstriert,
alle {{{app_m}}} grundsätzlich nur über {{{rs1}}} und {{{is1}}}
mit den {{{instr_m}}} kommunizieren zu lassen.
{{{rs1}}} und {{{is1}}} wären grundsätzlich zusammen mit der {{{app}}}
zu installieren und als Windows-Services
auf dem {{{ac}}}
zu starten.
Damit wäre die Benutzung lokal angeschlossener {{{instr_m}}}
ebenso möglich,
wie die Verwendung
entfernter, über virtuelle COM-Ports angeschlossener {{{instr_m}}}
und entfernter, über mehrere Instanzen des {{{is1}}} verwalteter {{{instr_m}}}.

Die lokal angeschlossenen {{{instr_m}}}
und die entfernten,
über virtuelle COM-Ports eingebundenen {{{instr_m}}}
werden dabei von der auf dem {{{ac}}} laufenden Instanz des {{{is1}}} verwaltet.

In jedem Fall könnte die {{{app}}} die Liste der verfügbaren {{{instr_m}}}
schnell über {{{rs1}}} und die Instanzen des {{{is1}}} beziehen.

Vorteile aus Anwendersicht sind:
- Konsistentes Verhalten der App.  Der Anwender wird nicht mit
  Verbindungstechnologien konfrontiert, sondern nur mit der Geräteliste.
- Kein Warten auf Geräteliste beim Start der {{{app}}}.
- Preisgünstige RS-232/TCP/IP-Konverter und andere preiswerte Hardware als
  alternative zur /IP Box/ verwendbar.

Ein wesentlicher Nachteil
dieser Kombination aus virtuellen COM-Ports
und {{{is1}}}
gegenüber einem rein von Instanzen von {{{is1}}}
gebildeten Netzwerk ist,
dass die Netzwerktopologie aus Sicht der {{{app}}} nicht mehr transparent ist.
Die über virtuelle COM-Ports vernetzten {{{instr_m}}}
erscheinen wie lokale,
direkt am {{{ac}}} angeschlossene {{{instr_m}}}.

** Notwendige Arbeiten
- Implementierung von {{{is1}}} und {{{rs1}}} als Windows-Services
- Implementierung der regelmäßigen Aktualisierung der Geräteliste am {{{is1}}}
- Anpassung von {{{dv}}}
- Anpassung von {{{rv}}}
- Anpassung der Softwareinstallation für alle {{{app_m}}}
