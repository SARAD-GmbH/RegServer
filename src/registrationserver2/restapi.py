...
class RestApi(Actor):
        ...
        try:
            for dir_entry in os.listdir(registrationserver2.FOLDER_AVAILABLE):
                ...
                if os.path.isfile(file):
                    ...
                    answer[dir_entry] = {
                        'Identification':
                        json.load(open(file)).get('Identification', None)
                    }
    ...
    @api.route(f'/{registrationserver2.PATH_AVAILABLE}/<did>/',
               methods=['GET'])
    def get_device(did):
    ...
    def get_history():
        ...
        try:
            for dir_entry in os.listdir(
                    f'{registrationserver2.FOLDER_HISTORY}'):
                if os.path.isfile(
    ...
    @staticmethod
    @api.route(f'/list/<did>/{registrationserver2.RESERVE_KEYWORD}',
               methods=['GET'])
    @api.route(
    ...
    def reserve_device(did):
        ...
        try:
            request_host = socket.gethostbyaddr(
    ...
    @staticmethod
    @api.route(f'/list/<did>/{registrationserver2.FREE_KEYWORD}',
               methods=['GET'])
    @api.route(
