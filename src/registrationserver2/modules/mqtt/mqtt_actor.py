...
class MqttActor(DeviceBaseActor):
    ...
    def __send__(self, msg: dict):
        ...
        send_satus = registrationserver2.actor_system.ask(SARAD_MQTT_CLIENT, 
                  {'CMD' : 'PUBLISH',
                   'Data' : {
    ...
    def __send_reserve__(self, msg):
        ...
        send_reserve_satus = registrationserver2.actor_system.ask(SARAD_MQTT_CLIENT, 
                  {'CMD' : 'PUBLISH',
                   'Data' : {
    ...
    def __send_free__(self, msg):
        ...
        send_free_satus = registrationserver2.actor_system.ask(SARAD_MQTT_CLIENT, 
                  {'CMD' : 'PUBLISH',
                   'Data' : {
                       'topic' : self.allowed_sys_topics['CTRL'],
    ...
    def __property_store__(self):
        ...
        for k in self.allowed_sys_topics.keys():
            self.allowed_sys_topics[k] = self.is_id + '/' + self.instr_id + self.allowed_sys_topics[k]
    ...
    def __init__(self):
        super.__init__()
        self.reserve_req_msg = { # how to get these information: App name, Host, User name???
            "Req": "reserve",
            "App": None,
            "Host": None,
            "User": None
