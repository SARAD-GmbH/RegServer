...
class MqttActor(DeviceBaseActor):
    ...
    def msg_analyzer(self, msg_rx : str): 
        ...
        if self.topic_buf_rx:
            if self.topic_buf_rx == self.sys_topic[2]: # meta data check, including reservation status
                if self.link_request == 1:
                    self.link_reply = 1
                    if self.pl_buf_rx:
                        self.link_accept = self.pl_buf_rx
                    else:
                        theLogger.info('Illegal reply \'None\' of reservation request!\n')
                        self.link_accept = 4
                else:
                    pass
                ...
                theLogger.info('Illegal message under the topic '+self.topic_buf_rx+'!\n')
    ...
    def on_message(self, client, userdata, message):
        ...
        if not self.mqtt_msg_rx.full():
            print('recieved message = %s', str(message.payload.decode("utf-8")))
    ...
    def mqtt_publish(self):
        theLogger.info('To publish the message (%s) under the topic (%s) with QoS=%s!\n', self.mqtt_payload, self.mqtt_topic, self.qos)
            ...
            if self.link_request_cnt < 3:
                if self.link_reply:
                    ...
                    if self.link_accept == 1:
                        ...
                        theLogger.info('The instrument with instrument ID ' + self.Instr_ID + ' is occupied!\n')
                        return 2
                    else:
                        self.link_request_cnt = 0
    ...
    signal.signal(signal.SIGINT, signal_handler)  # SIGINT: By default, interrupt is Ctrl+C 
    ...
    def __init__(self, is_id=str, instr_id=str, mqtt_actor_id=str):
        ...
        dirs = AppDirs("mqtt")
        for loc in [os.curdir, os.path.expanduser("~"), dirs.user_config_dir, dirs.site_config_dir]:
            try:
        ...
        if config == {}:
            theLogger.debug("There seems to be no configuration file. Using defaults.\n")
