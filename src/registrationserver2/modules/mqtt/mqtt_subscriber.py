...
class MqttParser(threading.Thread):
    def __init__(self, name):
        super().__init__()
        self.name = name
        logger.info(f"The thread ({self.name}) is created")
        self.len = 0
        self.topic_parts = []
...
class SaradMqttSubscriber(Actor):
    ...
    def _add_instr(self, msg: dict) -> dict:
        ...
        if msg.get("Data", None).get("payload", None) is None:
            return RETURN_MESSAGES.get("ILLEGAL_WRONGFORMAT")
        family_ = (
            msg.get("payload", None).get("Identification", None).get("Family", None)
        )
        ...
        with self.__lock:
            ...
            if data:
                ...
                setup_return = actor_system.ask(this_actor, {"CMD": "SETUP"})
    ...
    def _rm_instr(self, msg: dict) -> dict:
        ...
        with self.__lock:
            logger.info(
                f"[Del]:\tRemoved the instrument with Instrument ID: {instr_id} controled by the IS MQTT {is_id}"
            )
            link = fr"{self.__folder_available}{name_}"
        ...
        for mem in Instr_CONN_HISTORY:
            if mem.split("/")[0] == is_id:
                _re = self._rm_instr(
                    {
                        "CMD": "RM_DEVICE",
                        "Data": {"is_id": is_id, "instr_id": mem.split("/")[1]},
                    }
                )
                logger.info(_re)
        with self.__lock:
    ...
    def __publish(self, msg: dict) -> dict:
        ...
        if split_buf[2] == "control" and len(split_buf) == 3:
            if self.mqtt_payload.get("payload", None).get("Req", None) == "reserve":
                ...
                logger.info(f"Reserve target is: {split_buf[0]}/{split_buf[1]}")
