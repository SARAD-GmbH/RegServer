...
class DeviceActorManager(Actor):
    ...
    ACCEPTED_MESSAGES = {
        "CREATE":
        "__create__",  # is being called when the end-user-application wants to reserve the directly or indirectly connected device for exclusive communication, should return if a reservation is currently possible
        "ECHO":
...
DEVICE_ACTOR_MANAGER: ActorAddress = actor_system.createActor(
    DeviceActorManager,
    type(DeviceActorManager).__name__)
