...
def create_api_app(registrar):
    ...
    @api.deprecated
    @list_ns.route("/", endpoint="device_list")
    class List(Resource):
        # pylint: disable=too-few-public-methods
        ...
        def get(self):
            ...
            return get_device_statuses(registrar)

    @api.deprecated
    @list_ns.route("/<string:device_id>", endpoint="device_details")
    @list_ns.param(
        "device_id",
    ...
    class ListDevice(Resource):
        ...
        def get(self, device_id):
            ...
            return {device_id: get_device_status(registrar, device_id)}

    @api.deprecated
    @list_ns.route("/<string:device_id>/reserve", endpoint="device_reserve")
    @list_ns.param(
        "device_id",
        "ID of the connected device as gathered as key from the `/list` endpoint",
    )
    ...
    class ReserveDevice(Resource):
        ...
        def reserve_device(self, device_id, who, create_redirector):
            ...
            device_state = get_device_status_from_registrar(registrar, device_id)
            if (
    ...
    @api.deprecated
    @list_ns.route("/<string:device_id>/free", endpoint="device_free")
    @list_ns.param(
        "device_id",
        "ID of the connected device as gathered as key from the `/list` endpoint",
    )
    class FreeDevice(Resource):
        ...
        def get(self, device_id):
            ...
            if registrar is None:
                ...
                is_status = send_free_message(device_id, registrar)
            ...
            if status in (
                ...
                device_status = get_device_status_from_registrar(registrar, device_id)
    ...
    class Hosts(Resource):
        ...
        def get(self):
            ...
            return get_hosts(registrar)

    @hosts_ns.route("/<string:host>", endpoint="host_details")
    @hosts_ns.param(
        "host",
        "FQDN of the host",
    )
    ...
    @hosts_ns.route("/<string:host>/restart", endpoint="host_restart")
    @hosts_ns.param(
        "password",
        "This is not really a password since it is given here. "
        + "It is only there to prevent restart by mistake.",
    ...
    @hosts_ns.route("/<string:host>/scan", endpoint="host_scan")
    ...
    class Instruments(Resource):
        ...
        def get(self):
            ...
            response = []
            for device_id, device_status in get_device_statuses(registrar).items():
    ...
    class ReserveInstrument(Resource):
        ...
        def reserve_instr(self, instr_id, who, create_redirector):
            ...
            device_state = get_instr_status_from_registrar(registrar, instr_id)
            if device_state == {}:
                logger.error("Requested service not supported by actor system.")
                status = Status.NOT_SUPPORTED
            else:
    ...
    class FreeInstrument(Resource):
        ...
        def get(self, instr_id):
            ...
            if registrar is None:
                ...
                device_id = get_device_id(registrar, instr_id)
                is_status = send_free_message(device_id, registrar)
                if is_status == Status.CRITICAL:
                    status = Status.IS_NOT_FOUND
                else:
            ...
            if status in (
                ...
                device_status = get_instr_status_from_registrar(registrar, instr_id)
                logger.debug("After FREE registrar said: %s", device_status)
                return {
                    "Error code": status.value,
                    "Error": str(status),
    ...
    class GetValues(Resource):
        ...
        def get(self, device_id):
            ...
            device_actor = get_actor(registrar, device_id)
            ...
            if value_return.status in [
                ...
                if value_return.status == Status.OCCUPIED:
                    ...
                    notification = "We have to wait until the clock is set. This can take up to 59 s."
                else:
                    notification = ""
                return {
